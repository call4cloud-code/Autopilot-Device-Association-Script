...
`DeviceLink.ps1` is a PowerShell toolkit for exporting, inspecting, importing, applying, reading and removing a Windows Autopilot Device Association.
...
Place [`DeviceLink.ps1`](./DeviceLink.ps1) in a working folder on the test device. If Windows marked the downloaded file as coming from the internet, review it and unblock it:

```powershell
Unblock-File -Path .\DeviceLink.ps1
```

Open an elevated PowerShell session and view the built-in help:

```powershell
Get-Help .\DeviceLink.ps1 -Detailed
```
...
```powershell
.\DeviceLink.ps1 -Action Export -Verbose
```
...
```powershell
.\DeviceLink.ps1 -Action ReadAssociation -Verbose
```

### Preview local removal

```powershell
.\DeviceLink.ps1 -Action RemoveAssociation -WhatIf -Verbose
```

### Remove only the local UEFI association

```powershell
.\DeviceLink.ps1 -Action RemoveAssociation -Verbose
```
